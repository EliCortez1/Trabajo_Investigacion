...
<?php
	...
	if(getLoggedAdmin()){
		...
		if(!$spm_installed) echo Notification::show(array(
			'message' => '<i class="glyphicon glyphicon-info-sign"></i> 
¿Desea ofrecer a sus usuarios una experiencia de búsqueda más fácil y personalizada?<a href="https://bigprof.com/appgini/applications/search-page-maker-plugin-discount" target="_blank" class="alert-link"><i class="glyphicon glyphicon-hand-right"></i> Haga clic aquí para saber cómo puede ayudar el complemento Search Page Maker.</a>.',
			'dismiss_days' => 30,
			'class' => 'success',
			'id' => 'spm_notification'
		));
...
<div class="row hidden-sm hidden-xs" style="border-bottom: solid 2px #DDD;">
	<div class="col-md-2 col-md-offset-4 vspacer-lg"><strong>Campo filtrado
<?php echo $Translation['']; ?></strong></div>
	<div class="col-md-2 vspacer-lg"><strong>Operador de comparación<?php echo $Translation['']; ?></strong></div>
	<div class="col-md-2 vspacer-lg"><strong>Valor de comparación
<?php echo $Translation['']; ?></strong></div>
</div>
...
<?php
	for($i = 1; $i <= (3 * $FiltersPerGroup); $i++){ // Number of filters allowed
		...
		<div class="row FilterSet<?php echo $i; ?>" style="border-bottom: dotted 1px #DDD;">
			...
			<div class="col-md-1 text-right hidden-sm hidden-xs vspacer-md"><strong>Filtro<?php echo $Translation[""] . sprintf(" %02d", $i); ?></strong></div>
			<div class="col-md-1 hidden-md hidden-lg vspacer-md"><strong>Filtro<?php echo $Translation["filter"] . sprintf(" %02d", $i); ?></strong></div>
			...
			<div class="col-md-2 vspacer-md">
				<?php
					// Operators list
					$selop = new Combo;
					$selop->ListItem = array($Translation["Igual a"], $Translation["No igual a"], $Translation["Mayor que"], $Translation["Mayor qué o igual a"], $Translation["Menor que"], $Translation["Menor que o igual a"] , $Translation["Me gusta"] , $Translation["No me gusta"], $Translation["Está vacío"], $Translation["No esta vacío"]);
...
<div class="row" style="border-bottom: solid 2px #DDD;">
	<div class="col-md-offset-2 col-md-8 vspacer-lg"><strong>Ordenar por<?php echo $Translation['']; ?></strong></div>
</div>
...
<?php
	...
	for($i = 0; $i < $num_rules; $i++){
		...
		<div class="row" style="border-bottom: <?php echo $border_style; ?>;">
			...
			<div class="col-md-2 col-md-offset-2 vspacer-md hidden-xs hidden-sm text-right"><strong>Entonces por<?php echo ($i ? $Translation[''] : $Translation['']); ?></strong></div>
	...
	if(!$isAnonymous){
		...
		<div class="row filterByOwnership" style="border-bottom: solid 2px #DDD;">
			<div class="col-md-offset-2 col-md-8 vspacer-lg"><strong>
Registros para mostrar<?php echo $Translation['']; ?></strong></div>
		</div>
...
Solo tus propios registros<?php echo $Translation['']; ?>
					</label>
				</div>
				<div class="radio filterByOwnership">
					<label>
						<input type="radio" name="DisplayRecords" id="DisplayRecordsGroup" value="group"/>
						Todos los registros de su grupo.<?php echo $Translation['']; ?>
					</label>
				</div>
				<div class="radio filterByOwnership">
					<label>
...
<div class="row">
	<div class="col-md-2 col-md-offset-2 vspacer-lg">
		...
		<button type="submit" id="applyFilters" class="btn btn-success btn-block btn-lg"><i class="glyphicon glyphicon-ok"></i>
Aplicar filtros <?php echo $Translation['']; ?></button>
	</div>
	<?php if($this->AllowSavingFilters){ ?>
		<div class="col-md-3 vspacer-lg">
			<button type="submit" class="btn btn-default btn-block btn-lg" id="SaveFilter" name="SaveFilter_x" value="1"><i class="glyphicon glyphicon-align-left"></i> Guardar y aplicar filtros<?php echo $Translation['']; ?></button>
		</div>
	<?php } ?>
	<div class="col-md-2 vspacer-lg">
		<button onclick="jQuery('form')[0].reset();" type="submit" id="cancelFilters" class="btn btn-warning btn-block btn-lg"><i class="glyphicon glyphicon-remove"></i> Cancelar<?php echo $Translation['']; ?></button>
	</div>
</div>
